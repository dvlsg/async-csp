...
class FixedStack extends Stack {
  ...
  push(val) {
    if (!this.full()) return super.push(val)
  }
...
class FixedQueue extends Queue {
  ...
  push(val) {
    if (!this.full())
      // throw overflow? drop overflow? allow overflow?
...
class DroppingBuffer extends Queue {
  ...
  unshift(...vals) {
    ...
    if (this[ARR].length === 0 && vals[0]) this[ARR][0] = vals[0]
  }
...
class SlidingBuffer extends Queue {
  ...
  push(val) {
    ...
    this[ARR][this.tail] = val
    const overwrite = this.tail === this.head
