...
describe('Channel.empty', () => {
    ...
    it('should be false when puts queue is non empty', async() => {
        ...
        ch.put(1);
        ch.put(2);
        ch.buf._arr.length = 0; // is this necessary to test? should be a black box.
...
describe('Channel.put', () => {
    ...
    it('should put values in order', async() => {
        ...
        for (let val of [1,2,3,4,5])
            await ch.put(val);
        assert.equal(ch.buf._arr, [1,2,3,4,5]); // may not need to be tested, should be a "private" item.
    });
